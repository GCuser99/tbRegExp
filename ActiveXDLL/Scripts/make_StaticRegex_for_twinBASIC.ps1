# This script is only modified slightly from:
# https://github.com/sihlfall/vba-regex/blob/master/aio/make_aio.ps1
# -----Builds the aio in native twinBASIC format-----
# Changed the output module name to StaticRegex
# Changed the output file extension to ".twin"
# Redirected output file path
# Replaced VBA header with twinBASIC header
# Appended "End Module" at end of text stream 

param(
    [string]$outModuleName = "StaticRegex"
)
...
function CreateSingleFile {
    $outDirPath = ".\twinBASIC"
    $outFileName = $outModuleName + ".twin"


    $inDirPath = Resolve-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath "..\src")
    ...
    $commentSnippetPath = Resolve-Path -Path (Join-Path -Path $PSScriptRoot -ChildPath "..\aio\HeaderComment.bas")
    ...
    $oldBlock = 'Attribute VB_Name=(.+)'
    $newBlock =  'Private Module StaticRegex'
    $complete = $complete -replace $oldBlock, $newBlock
    
    # Close module structure
    $complete = $complete + $nl + 'End Module'
